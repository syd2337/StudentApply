...
<script type="text/javascript">
		...
		success:function(data){
			...
			$("#wrapper").empty();
				if (data == null) {
                    ...
                    +"<th scope='col'>课程资费</th><th scope='col'>课程次数</th></tr>";				
					var list = data;
					//alert(list1);
					for ( var i = 0; i < list.length; i++) {																		
						content = content
								+ '<tr align="center">'
								+ '<td><input type="checkbox" name="text_box" class="text_box" value=\''+list[i].id+'\' onclick="selectCourse(this,\''+list[i].rates+'\',\''+list[i].times+'\')"/></td>'					
								+ '<td>'+list[i].subjectName+'</td>'
								+ '<td>'+list[i].period+'</td>'
								+ '<td>'+list[i].rates +'元</td>'
	...
	function selectCourse(checkbox,rates,times){
		var val=$("#prices").val();
		if(val==""){
			val=0;
		}
		var price=parseInt(rates)*parseInt(times);
    ...
    if(courseList.length==0){
		...
		success:function(data){
			...
			if(data==null){
				...
				window.location.href='studentApplyNameAndPhone.html?name='+name+'&phoneNum='+phoneNumOne;
			};
